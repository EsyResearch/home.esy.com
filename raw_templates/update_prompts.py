                ...
                while j < len(lines) and j < i + 10:  # Look ahead up to 10 lines
                    if 'description:' in lines[j]:
                        ...
                        if not shortDescription_exists:
                            ...
                            updated_lines.append(f"{indent}  shortDescription: '{prompt_info['shortDescription']}',")
                            
                            # Format variables array
                            variables_str = ', '.join([f"'{var}'" for var in prompt_info['variables']])
                            updated_lines.append(f"{indent}  variables: [{variables_str}],")
...
def main():
    ...
    output_path = Path('/Users/lem/Desktop/Dev/apps/esy/client/esy.com/raw_templates/page_updated.tsx')
